...
const parseMRC = (line, lineParseParam, theCase, theAccount, theBUID) => {
	...
	if (/\S/.test(line)) { 
		if (/\[.+\]/.test(line)) {
			...
			if (! /^Basic Care|CARE|Success|Plus Package/.test(line) && 
			! /^ACD\/IVR/.test(line) &&
...
export function parseDescription(descrTxt, theCase, theAccount, theBUID, results) {
	...
	while (inputArr.length > 0) {
		...
		if(/^(SERVICES|Services|SERVICE|ADD SERVICE|ADD SERVICES):?$/.test(theLine)) {
				lineParser = parseMRC
		...
		const item = lineParser(theLine, lineParseParam, theCase, theAccount, theBUID)
	...
	if (insertVals.length > 0) {
		const descrSQL = "INSERT INTO nic_case_items ( accountID, BUID, sfdcCase, operation, skuid, sku, qtty, price ) VALUES \n\t"
