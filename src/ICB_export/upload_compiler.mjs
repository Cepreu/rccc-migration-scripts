...
const compileAccounts = (
  batchName,
  NgbsPackageId = 880,
  spendingLimitPercent = 1.5
) => {
  const sql = `
    SELECT
...
const compileRCMRSSummary = (batchName, invoiceDate = "2021-12-01") => {
  const sql = `
        ...
        FROM RCMRCSummary nic
            INNER JOIN BatchAccounts b ON nic.Account = b.UID
        WHERE b.batchID = ? 
            AND nic.InvoiceDate = ?
        `.replace(/\s+/g, " ");
