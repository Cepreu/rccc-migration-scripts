    ...
    INSERT INTO BatchEntitlements
      ...
      WITH mapping AS (
        ...
        SELECT DISTINCT
                ...
                e.MDURATION,
                e.CURRENCY_CODE,
                (e.RETAIL_PRICE-e.DISCOUNT) / CASE WHEN e.ProductFamily!='Overage' THEN e.MDURATION ELSE 1 END,
                e.QNTY_THRESHOLD,
                e.QNTY_THRESHOLD * ratio,
                lc.PRICE_USD,
                lc.PRICE_CAD,
                lc.PRICE_USD - (e.RETAIL_PRICE - e.DISCOUNT) / CASE WHEN e.ProductFamily!='Overage' THEN (e.MDURATION * ratio) ELSE 1 END AS discountUSD,
                lc.PRICE_CAD - (e.RETAIL_PRICE - e.DISCOUNT) / CASE WHEN e.ProductFamily!='Overage' THEN (e.MDURATION * ratio) ELSE 1 END AS discountCA,
                lc.NIC_PRICE,
                lc.ngbs_license_id,
                lc.PARENT,
                e.ProductFamily,
