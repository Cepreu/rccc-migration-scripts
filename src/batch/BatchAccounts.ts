...
import type { BatchDescription } from "./BatchDescription.js";

export class BatchAccounts {
  static defaultSpendingLimit = 1.5;
  readonly bd: BatchDescription;

  constructor(batchDescription: BatchDescription) {
    this.bd = batchDescription;
    console.log(JSON.stringify(this.bd));
  }
  static deleteProblematic(batchID: string, accountID: string | number) {
    db.prepare(
      "UPDATE BatchAccounts SET batchID=? WHERE batchID=? AND EID=?"
    ).run(`___${batchID}`, batchID, accountID);
  }
